...
export type OAuthRestClientInterface = {
  [Z in keyof typeof HttpMethod as Lowercase<Z>]: (
...
export const AmplienceRestClient = (config: {
  clientId: string;
  clientSecret: string;
  ...
  const request =
    (method: HttpMethod) =>
    async (config: AxiosRequestConfig | string): Promise<any> => {
      ...
      try {
        return await (
          await authenticatedAxios.request({method, ...config})
        ).data;
      } catch (error: any) {
        ...
        console.log(
          `Error while ${method}ing URL [ ${config.url} ]: ${error.message} ${error.code}`,
        );
      }
    };
